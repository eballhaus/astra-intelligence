...
def render_asset_card(row):
    import random
    symbol = row["symbol"]
    pred = get_predictions(symbol) if "get_predictions" in globals() else {
    ...
    with st.container():
        ...
        st.write(f"**Prediction:** ${pred['price']}  ({pred['percent']}%) in {pred['timeframe']}")
...
def render_advanced_chart():
    ...
    fig.add_trace(go.Scatter(x=df["t"], y=df["momentum"],
                             mode="lines", name="Momentum", line=dict(color="blue")))

    # Optional indicator toggles
    indicators = st.multiselect(
        "Show Indicators", ["EMA 20", "EMA 50", "RSI 14", "Bollinger Bands"], default=["EMA 20"]
    )
    if "EMA 20" in indicators:
        df["ema20"] = df["c"].ewm(span=20).mean()
        fig.add_trace(go.Scatter(x=df["t"], y=df["ema20"], name="EMA 20", line=dict(color="orange")))
    if "EMA 50" in indicators:
        df["ema50"] = df["c"].ewm(span=50).mean()
        fig.add_trace(go.Scatter(x=df["t"], y=df["ema50"], name="EMA 50", line=dict(color="purple")))
