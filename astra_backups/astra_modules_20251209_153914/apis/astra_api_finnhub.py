...
from astra_modules.api_keys import FINNHUB_API_KEY
from astra_modules.guardian.guardian_v6 import guardian_log


def get_data(symbol: str) -> pd.DataFrame:
    guardian_log(f"[Finnhub] 🔄 Fetching data for {symbol}")
    try:
        now = int(time.time())
        ...
        df["source"] = "Finnhub"
        guardian_log(f"[Finnhub] ✅ Retrieved {len(df)} rows for {symbol}")
        return df

    except Exception as e:
        guardian_log(f"[Finnhub] ⚠️ Error fetching {symbol}: {e}")
        return pd.DataFrame()
