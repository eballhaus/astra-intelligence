...
from astra_core.api_keys import TWELVEDATA_API_KEY
from astra_core.guardian.guardian_v6 import guardian


def get_data(symbol: str) -> pd.DataFrame:
    guardian.log(f"[TwelveData] 🔄 Fetching data for {symbol}")
    try:
        ...
        guardian.log(f"[TwelveData] ✅ Retrieved {len(df)} rows for {symbol}")
        return df

    except Exception as e:
        guardian.log(f"[TwelveData] ⚠️ Error fetching {symbol}: {e}")
        return pd.DataFrame()
