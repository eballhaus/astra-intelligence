...
class DataOrchestrator:
    ...
    def get_live_market_data(self, symbols=None):
        ...
        frames = []
        guardian.log(f"[DataOrchestrator] 🧠 Initiating live data pull for {len(symbols)} symbols.")
        ...
        for sym in symbols:
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    guardian.warn(f"[DataOrchestrator] ⚠️ Empty DataFrame returned for {sym}")
        ...
        # ------------------------------------------------------------------
        if not frames:
            guardian.error("[DataOrchestrator] ❌ No data frames returned from any API source.")
            return pd.DataFrame(columns=["symbol", "price", "change", "timestamp", "latency_s"])
