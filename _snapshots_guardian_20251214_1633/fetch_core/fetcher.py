...
import pandas as pd
import requests
from utils.df_cleaner import normalize_columns, strip_whitespace
...
def fetch_fmp(symbol, lookback_days):
    ...
    url = f"https://financialmodelingprep.com/api/v4/historical-price-full/{symbol}"
...
def fetch_alpha_vantage(symbol, lookback_days):
    ...
    params = {"function": "TIME_SERIES_DAILY_ADJUSTED", "symbol": symbol, "apikey": AV_KEY}
    r = requests.get(url, params=params, timeout=5)
    data = r.json()
...
def fetch_twelvedata(symbol, lookback_days):
    ...
    params = {"symbol": symbol, "interval": "1h", "apikey": TD_KEY, "outputsize": 500}
    r = requests.get(url, params=params, timeout=5)
    data = r.json()
...
def fetch_moralis(symbol, lookback_days):
    if not MORALIS_KEY: return None
...
def fetch_ohlcv(symbol: str, lookback_days: int):
    ...
    for name, api_func in apis:
        ...
        if df is not None and not df.empty:
            print(f"[FetchCore] ✅ Data fetched from {name} ({symbol}) — {len(df)} rows.")
            return df
        else:
