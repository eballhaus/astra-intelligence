...
from astra_core.guardian.environment_guardian import safe_import
...
def trace_tab_load(tab_name="tab_dashboard"):
    ...
    module = safe_import(f"astra_core.ui.{tab_name}")

    if module is None:
        print(f"❌ safe_import failed for astra_core.ui.{tab_name}")
        print("--------------------------------------------------")
        return
