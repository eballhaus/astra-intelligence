...
class AstraAPI:
    ...
    def get_data(self, symbol: str, *args, **kwargs) -> pd.DataFrame:
        ...
        guardian.log(f"[AstraAPI] 🔗 Fetching live data for {symbol} ...")
        ...
        try:
            ...
            guardian.log(f"[AstraAPI] ⚠️ Live data fetch failed for {symbol}: {e}")
    ...
    def _get_equity(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            series = (
                self.session.get(url, timeout=5).json().get("Time Series (5min)", {})
            )
            ...
            guardian.log(f"[AstraAPI] ⚠️ Alpha Vantage failed for {symbol}")
        ...
        try:
            ...
            guardian.log(f"[AstraAPI] ⚠️ FMP fallback failed for {symbol}")
        ...
        try:
            ...
            guardian.log(f"[AstraAPI] ⚠️ TwelveData fallback failed for {symbol}")
    ...
    def _get_crypto(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            price = float(data.get("usdPrice", 0)) if "usdPrice" in data else None
        except Exception:
            guardian.log(f"[AstraAPI] ⚠️ Moralis crypto fetch failed for {symbol}")
