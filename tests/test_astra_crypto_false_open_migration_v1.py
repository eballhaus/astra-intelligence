"""Tests for false crypto OPEN migration with broker-linked active predicate."""
...
class AstraCryptoFalseOpenMigrationTests(unittest.TestCase):
    def setUp(self):
    ...
    def _create_schema(self):
        ...
        cur.execute("""CREATE TABLE paper_positions (
            ...
            entry_price_source TEXT, source_bucket TEXT, broker_linked TEXT
        )""")
        conn.commit()
        conn.close()
    ...
    def _insert(self, **overrides):
        ...
        cols = "position_id,symbol,asset_type,status,entry_price,entry_price_verified,entry_price_provisional,entry_fill_id,entry_order_id,reconciliation_reason,prior_status,lifecycle_notes,source_candidate_id,lane_id,position_owner,quantity,entry_price_source,source_bucket,broker_linked"
    ...
    def test_false_crypto_open_is_migrated(self):
        self._insert()
        ...
        self.assertEqual(result["skipped_active_broker_linked"], 0)

    def test_real_broker_linked_crypto_position_is_not_migrated(self):
        # Real broker-linked active position that accidentally carries the
        # SIMULATED_OPEN_NO_BROKER_LINKAGE reconciliation reason.  It lacks an
        # entry_fill_id and entry_price_verified in the local record, but carries
        # an explicit broker_linked flag and nonzero quantity, so the canonical
        # active-position predicate must keep it from being migrated.
        self._insert(
            position_id="pos-real",
            entry_fill_id="",
            entry_price_verified=0,
            reconciliation_reason="SIMULATED_OPEN_NO_BROKER_LINKAGE",
            quantity=0.5,
            broker_linked="TRUE",
        )
        # Also insert a false OPEN row to ensure the migration still runs.
        self._insert(
            position_id="pos-false",
            entry_fill_id="",
            entry_price_verified=0,
            reconciliation_reason="SIMULATED_OPEN_NO_BROKER_LINKAGE",
            quantity=1.0,
        ...
        result = migrate_false_crypto_open(self.db_path, apply=True)
        self.assertEqual(result["migrated_this_run"], 1)
        self.assertEqual(result["skipped_active_broker_linked"], 1)
        self.assertEqual(result["remaining_eligible"], 0)

        conn = sqlite3.connect(self.db_path)
        cur = conn.cursor()
        cur.execute("SELECT status, position_id FROM paper_positions ORDER BY position_id")
        rows = {r[1]: r[0] for r in cur.fetchall()}
        conn.close()
        self.assertEqual(rows["pos-real"], "OPEN")
        self.assertEqual(rows["pos-false"], "SIMULATED")
