...
def debug_data_flow(symbol="AAPL"):
        ...
        if not df_api.empty:
            ...
            if 'close' in df_api.columns:
                ...
                if symbol == "AAPL" and (avg_price < 100 or avg_price > 300):
                    realistic = False
                    print(f"   ⚠️ AAPL price unrealistic! Expected $150-250, got ${avg_price:.2f}")
    ...
    try:
        ...
        if not df_dash.empty:
            ...
            print(f"   First close: {df_dash['close'].iloc[0] if 'close' in df_dash.columns else 'MISSING'}")
            print(f"   Last close: {df_dash['close'].iloc[-1] if 'close' in df_dash.columns else 'MISSING'}")
            ...
            if 'close' in df_dash.columns:
                ...
                if unique_prices == 1:
                    print(f"   ⚠️ Only one price value - likely synthetic!")
    except Exception as e:
    ...
    try:
        ...
        if not df_unified.empty:
            if hasattr(df_unified, 'attrs'):
                print(f"   Source from attrs: {df_unified.attrs.get('source', 'NOT SET')}")
            print(f"   Latest price: {df_unified['close'].iloc[-1] if 'close' in df_unified.columns else 'MISSING'}")
...
def main():
    ...
    env_vars = ["ASTRA_API_BASE", "ASTRA_API_KEY", "ASTRA_BACKEND_URL", "ASTRA_LIVE_MODE"]
    for var in env_vars:
        value = os.getenv(var)
