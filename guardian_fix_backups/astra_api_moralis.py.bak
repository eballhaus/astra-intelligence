...
def get_data(symbol: str) -> pd.DataFrame:
    guardian_log(f"[Moralis] 🔄 Fetching data for {symbol}")
    try:
        ...
        df["source"] = "Moralis"
        guardian_log(f"[Moralis] ✅ Retrieved {len(df)} rows for {symbol}")
        return df

    except Exception as e:
        guardian_log(f"[Moralis] ⚠️ Error fetching {symbol}: {e}")
        return pd.DataFrame()
