...
import threading
import time
from astra_dashboard.core.guardian.guardian_secure_api import GuardianSecureAPI
...
class GuardianStreamManager:
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        print(
            f"[GuardianStream] 🔌 Connection closed ({close_status_code}): {close_msg}"
        )
    ...
    def _connect(self):
        ...
        url = "wss://ws.finnhub.io?token=your_key_here"
        ...
        if isinstance(data, dict) and data.get("event") == "price":
            print(f"[GuardianStream] 💰 Price event detected: {data}")
            pass
        ...
        try:
            ...
            if "price" in data:
                print(
                    f"[GuardianStream] 💹 Live update: {data.get('symbol')} @ {data.get('price')}"
                )
                if self.on_update:
                    print("[GuardianStream] 🔁 Triggering LearningEngine.on_update()")
                    self.on_update(data)
