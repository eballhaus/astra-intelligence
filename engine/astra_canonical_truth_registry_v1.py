...
CANONICAL_FACTS_V1: dict[str, dict[str, Any]] = {
    ...
    "LOCAL_OPEN_SWING_EQUITY_POSITION_COUNT": _fact("LOCAL_OPEN_SWING_EQUITY_POSITION_COUNT", "PaperAutopilot canonical position store", "PaperAutopilotEngine.db_path", "canonical open-position reader", "OPEN + SWING_EQUITY", "position_id", ["historical positions"]),
    ...
    "LOCAL_OPEN_DAY_EQUITY_POSITION_COUNT": _fact("LOCAL_OPEN_DAY_EQUITY_POSITION_COUNT", "PaperAutopilot canonical position store", "PaperAutopilotEngine.db_path", "canonical open-position reader", "OPEN + DAY_EQUITY", "position_id", ["historical positions"]),
    ...
    "LOCAL_OPEN_ETF_POSITION_COUNT": _fact("LOCAL_OPEN_ETF_POSITION_COUNT", "PaperAutopilot canonical position store", "PaperAutopilotEngine.db_path", "canonical open-position reader", "OPEN + ETF", "position_id", ["ranking rows"]),
    ...
    # Per-pair observability is bounded by the worker's monitored crypto
    # universe.  It is diagnostic evidence, never a trading approval.
    "CRYPTO_PAIR_QUOTE_OBSERVABLE": _fact("CRYPTO_PAIR_QUOTE_OBSERVABLE", "PaperAutopilotWorker", "state/paper_autopilot_state.json", "crypto_rankings_snapshot_v1.crypto_quote_integrity_rows", "most recently evaluated monitored crypto pair has a provider quote", "symbol", ["ranking action", "last-price cache"]),
    "CRYPTO_PAIR_LAST_SUCCESSFUL_QUOTE_AT": _fact("CRYPTO_PAIR_LAST_SUCCESSFUL_QUOTE_AT", "PaperAutopilotWorker", "state/astra_system_integrity_scanner_v1.json", "crypto market-data observability", "bounded last successful quote timestamp by monitored pair", "symbol", ["snapshot generated_at"]),
    "CRYPTO_PAIR_QUOTE_FAILURE_STREAK": _fact("CRYPTO_PAIR_QUOTE_FAILURE_STREAK", "PaperAutopilotWorker", "state/astra_system_integrity_scanner_v1.json", "crypto market-data observability", "bounded consecutive failed quote observations by monitored pair", "symbol", ["historical candidate count"]),
    "CRYPTO_PAIR_BID_AVAILABLE": _fact("CRYPTO_PAIR_BID_AVAILABLE", "ProviderRouter", "state/paper_autopilot_state.json", "crypto_rankings_snapshot_v1.crypto_quote_integrity_rows", "current evaluated crypto pair bid presence", "symbol", ["last price", "bar low"]),
    "CRYPTO_PAIR_ASK_AVAILABLE": _fact("CRYPTO_PAIR_ASK_AVAILABLE", "ProviderRouter", "state/paper_autopilot_state.json", "crypto_rankings_snapshot_v1.crypto_quote_integrity_rows", "current evaluated crypto pair ask presence", "symbol", ["last price", "bar high"]),
    "CRYPTO_PAIR_SPREAD_AVAILABLE": _fact("CRYPTO_PAIR_SPREAD_AVAILABLE", "ProviderRouter", "state/paper_autopilot_state.json", "crypto_rankings_snapshot_v1.crypto_quote_integrity_rows", "current evaluated crypto pair real bid/ask spread availability", "symbol", ["zero default", "bar range"]),
    "CRYPTO_PAIR_COMPLETED_VOLUME_AVAILABLE": _fact("CRYPTO_PAIR_COMPLETED_VOLUME_AVAILABLE", "AlpacaPaperBroker historical bars", "state/paper_autopilot_state.json", "crypto_rankings_snapshot_v1.crypto_volume_audit", "rolling completed crypto bar volume availability", "symbol", ["quote volume", "fabricated zero"]),
    "CRYPTO_PAIR_DATA_QUALITY_READY": _fact("CRYPTO_PAIR_DATA_QUALITY_READY", "candidate_execution_integrity_v1", "state/crypto_operational_integrity_readiness_v1.json", "crypto candidate readiness", "current evaluated crypto pair data-quality gate result", "symbol", ["ranking action", "lane state"]),
    "CRYPTO_ROTATION_LAST_EVALUATED_AT": _fact("CRYPTO_ROTATION_LAST_EVALUATED_AT", "PaperAutopilotWorker", "state/paper_autopilot_state.json", "crypto_rankings_snapshot_v1", "last bounded evaluation timestamp per monitored crypto pair", "symbol", ["capability generated_at"]),
    "CRYPTO_ROTATION_CYCLE_COMPLETION": _fact("CRYPTO_ROTATION_CYCLE_COMPLETION", "PaperAutopilotWorker", "state/paper_autopilot_state.json", "crypto_rankings_snapshot_v1", "bounded crypto discovery rotation progress", "rotation cursor", ["ranking cache age"]),
}
