...
def build_legacy_swing_required_evidence_v1(position: Mapping[str, Any], baseline: Mapping[str, Any], *, now: datetime | None = None) -> dict[str, Any]:
    ...
    context_provider = str(fmp_context.get("provider") or "FMP").upper()
    context_source = f"{context_provider}.company_profile"
    ...
    if direct_thesis in {"INTACT", "WEAKENING", "MATERIALLY_DETERIORATED", "BROKEN", "CONFLICTING"}:
    ...
    elif str(fmp_context.get("response_state") or "").upper() in {"SUCCESS", "PARTIAL"} and dict(fmp_context.get("normalized_fields") or {}):
        ...
        thesis = record(
            ...
            source=context_source, fmp_record_id=fmp_context.get("record_id"),
        ...
        thesis = record(
            ...
            [f"stale_{context_provider.lower()}_profile_context"], thesis_state="UNKNOWN", direct_evidence=False,
            source=context_source, fmp_record_id=fmp_context.get("record_id"),
            endpoint_family=fmp_context.get("endpoint_family"),
        )
    else:
        ...
        thesis = record("THESIS_STATE", "UNAVAILABLE", "UNKNOWN", 0.0, [limitation], thesis_state="UNKNOWN", direct_evidence=False,
                        source=context_source if fmp_context else "PaperAutopilot.broker_position_snapshot",
