...
function FilterBar({ filter, onChange, rows }) {
  const options = [
    ["ALL", "All"], ["OPPORTUNITIES", "Opportunities"], ["CURRENT_POSITIONS", "Current Positions"], ["NEEDS_ATTENTION", "Needs Attention"],
    ["BUY_NOW", "Buy Now"], ["WATCH", "Watch"], ["HOLD", "Hold"], ["LOSING_MOMENTUM", "Losing Momentum"], ["PROTECT_PROFIT", "Protect Profit"],
    ["APPROACHING_SELL", "Approaching Sell"], ["SELL_RECOMMENDED", "Sell Recommended"], ["PAPER_ELIGIBLE", "Paper Eligible"], ["BLOCKED", "Blocked"], ["STALE", "Stale"],
  ];
  return (
...
function ActionCard({ row, selected, onSelect }) {
  ...
  return (
    <button type="button" className={`copilot-action-card ${selected ? "selected" : ""} ${urgent ? "urgent" : ""}`} onClick={() => onSelect(row.recommendation_id)} aria-pressed={selected}>
      ...
      <div className="copilot-symbol-row"><strong>{text(row.symbol)}</strong><span>{titleCase(row.asset_type || row.asset_class || "equity")}</span></div>
      ...
      <div className="copilot-card-footer"><span>{titleCase(row.evidence_quality)}</span><span>{titleCase(row.freshness)}</span><span>{row.paper_autopilot_eligible ? "Paper eligible" : "Advisory only"}</span></div>
    </button>
  );
}
    ...
    <section className="copilot-workspace" aria-label={`Decision workspace for ${row.symbol}`}>
      ...
      <div className="copilot-workspace-columns">
        ...
        <div>
          ...
          <section className="copilot-detail-block"><h3>What changed</h3><div className="copilot-unavailable">{Array.isArray(row.recommendation_history) && row.recommendation_history.length ? "Recommendation history is available in the detailed history view." : "Recommendation history is unavailable in this payload."}</div></section>
        </div>
      </div>
...
export default function CopilotPage({ selectedSymbol = "", onSelectSymbol }) {
  ...
  return (
    <div className="copilot-page">
      ...
      <FilterBar filter={filter} onChange={setFilter} rows={rows} />
