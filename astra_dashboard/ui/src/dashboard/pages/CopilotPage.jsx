...
function ActionCard({ row, selected, onSelect }) {
  ...
  return (
    <button type="button" className={`copilot-action-card ${selected ? "selected" : ""} ${urgent ? "urgent" : ""}`} onClick={() => onSelect(row.recommendation_id)} aria-pressed={selected}>
      ...
      <div className="copilot-card-meta"><span>{formatConfidence(row.confidence) || "Unavailable"}{present(row.confidence) ? " confidence" : ""}</span><span>{titleCase(row.trade_style || row.horizon)}</span><span>{titleCase(row.preferred_horizon || row.horizon)}</span></div>
...
function ComparisonWorkspace({ baseRow, rows, compareId, onCompare }) {
  ...
  if (!baseRow) return null;
    const fields = [
      ["Recommendation", (row) => titleCase(stateLabel(row))], ["Confidence", (row) => formatConfidence(row.confidence)], ["Evidence", (row) => titleCase(row.evidence_quality)],
...
function DecisionWorkspace({ row }) {
  ...
  return (
    <section className="copilot-workspace" aria-label={`Decision workspace for ${row.symbol}`}>
      ...
      <div className="copilot-workspace-metrics"><Metric label="Confidence" value={formatConfidence(row.confidence)} tone="blue" hint="Confidence is distinct from evidence quality" /><Metric label="Evidence" value={titleCase(row.evidence_quality)} tone={row.evidence_quality === "INSUFFICIENT_EVIDENCE" ? "warn" : "neutral"} /><Metric label="Freshness" value={titleCase(row.freshness)} /><Metric label="Trade style" value={titleCase(row.trade_style || row.horizon)} /><Metric label="Horizon" value={titleCase(row.preferred_horizon || row.horizon)} /></div>
      ...
      <div className="copilot-workspace-columns">
        ...
        <div>
          <section className="copilot-detail-block"><h3>Conviction and evidence</h3><div className="copilot-evidence-grid"><Metric label="Overall confidence" value={formatConfidence(row.confidence)} tone="blue" /><Metric label="Symbol evidence" value={titleCase(row.symbol_evidence_quality)} /><Metric label="Risk" value={titleCase(row.risk_level)} /><Metric label="Liquidity" value={titleCase(row.liquidity)} /></div>{contextValues.length ? <div className="copilot-context-grid">{contextValues.map(([label, value]) => <ContextItem key={label} label={label} value={value} />)}</div> : <div className="copilot-unavailable">Technical, market, sector, catalyst, and fundamental context is unavailable in this cached recommendation.</div>}</section>
