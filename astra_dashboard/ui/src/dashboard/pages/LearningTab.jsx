...
export default function LearningTab({ compact = false }) {
  ...
  const fullDiagnosticSnapshotText = () => [
    ...
    JSON.stringify({
      ...
      astra_adaptive_occupancy_evolution_suite_v1: astraAdaptiveOccupancyEvolutionSuite,
  ...
  const tier4SectionText = (label, key) => [
    `ASTRA TIER 4 ${label.toUpperCase()}`,
    `Generated: ${String(unified?.generated_at || lastFetchAt || "n/a")}`,
    JSON.stringify(astraAdaptiveOccupancyEvolutionSuite?.[key] || {}, null, 2),
  ].join("\n");
      ...
      <details style={{ ...panelStyle }} open={false}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 8, marginTop: 10, fontSize: 12 }}>
          {[
            ...
            ["Effective Learning Occupancy", astraAdaptiveOccupancyEvolutionSuite?.effective_learning_capacity_v1?.effective_learning_occupancy],
            ["Effective Learning Room", astraAdaptiveOccupancyEvolutionSuite?.effective_learning_capacity_v1?.effective_learning_capacity_available],
            ["Exit Review Candidates", safeNumber(astraAdaptiveOccupancyEvolutionSuite?.exit_decision_intelligence_v1?.exit_review_candidates?.length).toFixed(0)],
            ["Capacity Pipeline", astraAdaptiveOccupancyEvolutionSuite?.adaptive_capacity_utilization_pipeline_v1?.effective_capacity_pipeline_status],
        ...
        <details style={{ marginTop: 12, background: "rgba(9,26,48,0.72)", border: "1px solid #34577f", borderRadius: 12, padding: "10px 12px" }}>
          <summary style={{ cursor: "pointer", color: "#dce9fb", fontWeight: 900 }}>
            Tier 1-2 Profitability Activation & Trade Management Intelligence: {String(astraTier12ProfitabilityActivation?.status || "warming up").replaceAll("_", " ")}
          </summary>
          <div style={{ color: "#9fb1cc", fontSize: 12, marginTop: 8 }}>
            Verifies propagation from canonical lessons into advisory trade-management, Paper influence, Shadow transfer, Cortex, Ask Astra, and Learning Center diagnostics. This remains diagnostic-only and does not change trading behavior.
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8, marginTop: 10, fontSize: 12 }}>
            {[
              ["Cortex Propagation", safeNumber(astraTier12ProfitabilityActivation?.cortex_autonomous_oversight_propagation_verification_v1?.cortex_propagation_verification_score).toFixed(1)],
              ["Trade Mgmt Fabric", safeNumber(astraTier12ProfitabilityActivation?.trade_management_intelligence_fabric_v1?.trade_management_intelligence_fabric_score).toFixed(1)],
              ["Lesson Consumption", safeNumber(astraTier12ProfitabilityActivation?.canonical_lesson_propagation_consumer_completion_v2?.lesson_consumption_score_after).toFixed(1)],
              ["Profit Capture", safeNumber(astraTier12ProfitabilityActivation?.profit_capture_consumer_wiring_completion_v2?.profit_capture_score_after).toFixed(1)],
              ["Exit Learning", safeNumber(astraTier12ProfitabilityActivation?.exit_learning_consumer_wiring_completion_v2?.exit_learning_score_after).toFixed(1)],
              ["Paper Influence", safeNumber(astraTier12ProfitabilityActivation?.paper_decision_influence_wiring_v2?.paper_decision_influence_score_after).toFixed(1)],
              ["Shadow Transfer", safeNumber(astraTier12ProfitabilityActivation?.shadow_to_paper_transfer_intelligence_v2?.shadow_to_paper_transfer_score_after).toFixed(1)],
              ["Symbol Intelligence", safeNumber(astraTier12ProfitabilityActivation?.symbol_satellite_compression_behavioral_memory_v2?.symbol_satellite_compression_score).toFixed(1)],
              ["Ranking Reconstruction", safeNumber(astraTier12ProfitabilityActivation?.ranking_attribution_reconstruction_v2?.ranking_reconstruction_score_after).toFixed(1)],
              ["Learning ROI", safeNumber(astraTier12ProfitabilityActivation?.learning_roi_engine_v2?.learning_roi_score_after).toFixed(1)],
              ["Intelligence Use", safeNumber(astraTier12ProfitabilityActivation?.mandatory_final_audit_v1?.before_after_metrics?.intelligence_utilization_score?.[1]).toFixed(1)],
              ["Cortex Integration", safeNumber(astraTier12ProfitabilityActivation?.cortex_integration_contract_v2?.cortex_integration_score_after).toFixed(1)],
              ["Highest ROI", astraTier12ProfitabilityActivation?.mandatory_final_audit_v1?.highest_roi_next_improvement],
              ["Top Blocker", astraTier12ProfitabilityActivation?.mandatory_final_audit_v1?.top_5_bottlenecks?.[0]],
            ].map(([label, value]) => (
              <div key={label} style={{ background: "rgba(12,24,42,0.46)", border: "1px solid #2f4a72", borderRadius: 8, padding: "8px 10px" }}>
                <div style={{ color: "#7da3d6", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</div>
                <strong style={{ color: "#f2f7ff" }}>{String(value ?? "warming up").replaceAll("_", " ")}</strong>
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
            {[
              ["Cortex Propagation Verification", astraTier12ProfitabilityActivation?.cortex_autonomous_oversight_propagation_verification_v1],
              ["Trade Management Fabric", astraTier12ProfitabilityActivation?.trade_management_intelligence_fabric_v1],
              ["Canonical Propagation", astraTier12ProfitabilityActivation?.canonical_lesson_propagation_consumer_completion_v2],
              ["Profit Capture Wiring", astraTier12ProfitabilityActivation?.profit_capture_consumer_wiring_completion_v2],
              ["Exit Learning Wiring", astraTier12ProfitabilityActivation?.exit_learning_consumer_wiring_completion_v2],
              ["Paper Decision Influence", astraTier12ProfitabilityActivation?.paper_decision_influence_wiring_v2],
              ["Shadow-to-Paper Transfer", astraTier12ProfitabilityActivation?.shadow_to_paper_transfer_intelligence_v2],
              ["Symbol Behavioral Memory", astraTier12ProfitabilityActivation?.symbol_satellite_compression_behavioral_memory_v2],
              ["Sector / Index Personality", astraTier12ProfitabilityActivation?.sector_index_personality_engine_v2],
              ["Historical Recovery", astraTier12ProfitabilityActivation?.historical_trade_intelligence_recovery_v2],
              ["Ranking Reconstruction", astraTier12ProfitabilityActivation?.ranking_attribution_reconstruction_v2],
              ["Candidate Ledger Join", astraTier12ProfitabilityActivation?.candidate_ledger_outcome_join_engine_v1],
              ["Learning ROI V2", astraTier12ProfitabilityActivation?.learning_roi_engine_v2],
              ["Cortex Integration", astraTier12ProfitabilityActivation?.cortex_integration_contract_v2],
              ["Final Audit", astraTier12ProfitabilityActivation?.mandatory_final_audit_v1],
            ].map(([label, row]) => (
              <details key={label} style={{ background: "rgba(10,22,41,0.48)", border: "1px solid #29476f", borderRadius: 10, padding: "8px 10px" }}>
                <summary style={{ cursor: "pointer", color: "#dce9fb", fontWeight: 800 }}>
        ...
        <details style={{ marginTop: 12, background: "rgba(9,26,48,0.72)", border: "1px solid #34577f", borderRadius: 12, padding: "10px 12px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8, marginTop: 10, fontSize: 12 }}>
            {[
              ...
              ["Broker Truth Records", safeNumber(astraPerformanceConversionExitBrokerFmpTruthCapacity?.broker_confirmed_truth_records).toFixed(0)],
              ["Lifecycle Attribution", safeNumber(astraPerformanceConversionExitBrokerFmpTruthCapacity?.lifecycle_attribution_records).toFixed(0)],
              ["Metric Status", astraPerformanceConversionExitBrokerFmpTruthCapacity?.verified_trading_metrics_status || "insufficient broker truth evidence"],
              ["True Paper PF", astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_profit_factor === null || astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_profit_factor === undefined ? "blocked pending broker truth" : astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_profit_factor],
              ["True Paper WR", astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_win_rate === null || astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_win_rate === undefined ? "blocked pending broker truth" : `${safeNumber(astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_win_rate).toFixed(1)}%`],
              ["True Avg Return", astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_avg_return === null || astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_avg_return === undefined ? "blocked pending broker truth" : `${safeNumber(astraPerformanceConversionExitBrokerFmpTruthCapacity?.true_paper_avg_return).toFixed(2)}%`],
              ["Evidence Inflation", astraPerformanceConversionExitBrokerFmpTruthCapacity?.evidence_inflation_detected ? "detected and guarded" : "not detected"],
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
            {[
              ...
              ["Truth Integrity Audit", astraPerformanceConversionExitBrokerFmpTruthCapacity?.truth_integrity_audit_v1],
        ...
        <details style={{ marginTop: 12, background: "rgba(9,26,48,0.72)", border: "1px solid #34577f", borderRadius: 12, padding: "10px 12px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8, marginTop: 10, fontSize: 12 }}>
            {[
              ...
              ["Broker Truth Records", safeNumber(astraPaperProviderCortexCompletion?.broker_confirmed_truth_records).toFixed(0)],
              ["Lifecycle Attribution", safeNumber(astraPaperProviderCortexCompletion?.lifecycle_attribution_records).toFixed(0)],
              ["Attributed Paper Trades", safeNumber(astraPaperProviderCortexCompletion?.attributed_paper_trade_count).toFixed(0)],
              ["Metric Status", astraPaperProviderCortexCompletion?.verified_trading_metrics_status || "insufficient broker truth evidence"],
              ["Official Paper PF", astraPaperProviderCortexCompletion?.paper_profit_factor === null || astraPaperProviderCortexCompletion?.paper_profit_factor === undefined ? "blocked pending broker truth" : safeNumber(astraPaperProviderCortexCompletion?.paper_profit_factor).toFixed(2)],
              ["Official Win Rate", astraPaperProviderCortexCompletion?.paper_win_rate === null || astraPaperProviderCortexCompletion?.paper_win_rate === undefined ? "blocked pending broker truth" : `${safeNumber(astraPaperProviderCortexCompletion?.paper_win_rate).toFixed(1)}%`],
              ["Official Avg Return", astraPaperProviderCortexCompletion?.paper_average_return === null || astraPaperProviderCortexCompletion?.paper_average_return === undefined ? "blocked pending broker truth" : `${safeNumber(astraPaperProviderCortexCompletion?.paper_average_return).toFixed(2)}%`],
              ["Diagnostic Lifecycle PF", safeNumber(astraPaperProviderCortexCompletion?.diagnostic_lifecycle_profit_factor).toFixed(2)],
              ["Diagnostic Lifecycle WR", `${safeNumber(astraPaperProviderCortexCompletion?.diagnostic_lifecycle_win_rate).toFixed(1)}%`],
              ["Duplicate Records", safeNumber(astraPaperProviderCortexCompletion?.duplicate_record_count).toFixed(0)],
              ["Evidence Inflation", astraPaperProviderCortexCompletion?.evidence_inflation_detected ? "detected and guarded" : "not detected"],
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
            {[
              ...
              ["Truth Integrity Audit", astraPaperProviderCortexCompletion?.truth_integrity_audit_v1],
        ...
        <details style={{ marginTop: 12, background: "rgba(9,26,48,0.72)", border: "1px solid #34577f", borderRadius: 12, padding: "10px 12px" }}>
          <summary style={{ cursor: "pointer", color: "#dce9fb", fontWeight: 900 }}>
            Intelligence Consumption & Utilization: {String(astraIntelligenceConsumptionLayer?.status || "warming up").replaceAll("_", " ")}
          </summary>
          <div style={{ color: "#9fb1cc", fontSize: 12, marginTop: 8 }}>
            Tracks whether collected intelligence is actually consumed by Paper advisory records, Copilot, replay, horizon, profit capture, and Cortex attribution. This is diagnostic-only and uses the existing unified diagnostics payload.
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8, marginTop: 10, fontSize: 12 }}>
            {[
              ...
              ["Paper Trust", astraPaperProviderCortexCompletion?.paper_metric_trust_level],
              ["Open Cortex Issues", safeNumber(astraPaperProviderCortexCompletion?.cortex_open_issues).toFixed(0)],
              ["Behavior Safe", astraIntelligenceConsumptionLayer?.behavior_safe_to_apply ? "true" : "false"],
            ].map(([label, value]) => (
              <div key={label} style={{ background: "rgba(12,24,42,0.46)", border: "1px solid #2f4a72", borderRadius: 8, padding: "8px 10px" }}>
                <div style={{ color: "#7da3d6", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</div>
                <strong style={{ color: "#f2f7ff" }}>{String(value ?? "warming up").replaceAll("_", " ")}</strong>
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
            {[
              ...
              ["Historical Replay Expansion", astraPaperProviderCortexCompletion?.historical_replay_expansion_validation_v2],
              ["Horizon Usage Validation", astraPaperProviderCortexCompletion?.horizon_usage_paper_influence_validation_v1],
            ].map(([label, row]) => (
              <details key={label} style={{ background: "rgba(10,22,41,0.48)", border: "1px solid #29476f", borderRadius: 10, padding: "8px 10px" }}>
                <summary style={{ cursor: "pointer", color: "#dce9fb", fontWeight: 800 }}>
                  {label}: {Array.isArray(row) ? `${row.length} item(s)` : String(row?.status || row?.utilization_status || row?.provider_recovery_status || "available").replaceAll("_", " ")}
                </summary>
                <pre style={{ margin: "8px 0 0", color: "#9fb1cc", whiteSpace: "pre-wrap", fontSize: 11, maxHeight: 260, overflow: "auto" }}>
                  {JSON.stringify(row || {}, null, 2)}
                </pre>
        ...
        <details style={{ marginTop: 12, background: "rgba(9,26,48,0.72)", border: "1px solid #34577f", borderRadius: 12, padding: "10px 12px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8, marginTop: 10, fontSize: 12 }}>
            {[
              ...
              ["Exit Type Validation", safeNumber(astraProfitExitRankingCopilotConvergence?.attribution_closure_exit_type_capture_validation_v1?.exit_type_capture_validation_score ?? astraProfitExitRankingCopilotConvergence?.exit_type_capture_validation?.exit_type_capture_validation_score).toFixed(1)],
              ["Closed Trade Persistence", safeNumber(astraProfitExitRankingCopilotConvergence?.attribution_closure_exit_type_capture_validation_v1?.closed_trade_attribution_persistence_score ?? astraProfitExitRankingCopilotConvergence?.closed_trade_attribution_persistence?.score).toFixed(1)],
              ...
              ["Copilot Readiness", safeNumber(astraProfitExitRankingCopilotConvergence?.copilot_attribution_intelligence_v1?.copilot_readiness_score ?? astraProfitExitRankingCopilotConvergence?.copilot_attribution_intelligence_v1?.recommendation_quality_score).toFixed(1)],
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
            {[
              ...
              ["Exit Type Capture Validation", astraProfitExitRankingCopilotConvergence?.attribution_closure_exit_type_capture_validation_v1 || astraProfitExitRankingCopilotConvergence?.exit_type_capture_validation],
        ...
        <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
          {[
            ...
            ["Horizon Diversity Without Quotas", "horizon_diversity_without_quotas_v1"],
            ["Autonomous Trading Governance", "autonomous_trading_governance_v1"],
            ["Shadow to Paper Feedback", "shadow_paper_feedback_connection_v1"],
            ["Trading Completion Verification", "trading_intelligence_completion_behavior_verification_v1"],
