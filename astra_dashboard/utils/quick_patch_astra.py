...
tab_dashboard = root / "core/ui/dashboard/tab_dashboard.py"
if tab_dashboard.exists():
    txt = tab_dashboard.read_text()
    if "from core.guardian import guardian as guardian" not in txt:
        txt = txt.replace(
            "import guardian", "from core.guardian import guardian as guardian"
        )
        if "from core.guardian import guardian" not in txt:
            txt = "from core.guardian import guardian as guardian\n" + txt
        tab_dashboard.write_text(txt)
        print("✅ Patched guardian import in tab_dashboard.py")

# --- schema_validator fallback ---
sv = root / "core/guardian/schema_validator.py"
if not sv.exists():
    sv.write_text(
