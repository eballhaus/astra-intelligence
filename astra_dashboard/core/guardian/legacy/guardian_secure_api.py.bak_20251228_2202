...
def __init__(self):
    ...
    # ---------------------------------------------------------
    # Dashboard bundle for Astra Intelligence UI
    # ---------------------------------------------------------
    _cache = {"timestamp": 0, "data": None}

    def get_dashboard_bundle(self):
        """Return a cached 12-asset dashboard snapshot (6 stocks + 6 crypto)."""
        import time
        import json

        # Reuse cache if under 60s old
        if time.time() - self._cache["timestamp"] < 60 and self._cache["data"]:
            return self._cache["data"]

        try:
            stock_symbols = ["AAPL", "MSFT", "NVDA", "AMZN", "GOOG", "META"]
            crypto_symbols = ["BTC-USD", "ETH-USD", "SOL-USD", "ADA-USD", "AVAX-USD", "DOGE-USD"]

            stocks = [self.fetch_stock(sym) for sym in stock_symbols]
            cryptos = [self.fetch_crypto(sym) for sym in crypto_symbols]
            ...
            # Cache result
            self._cache = {"timestamp": time.time(), "data": bundle}
            return bundle

        except Exception as e:
            print(f"[GuardianSecureAPI] ⚠️ Bundle fetch failed: {e}")
            # Return last known cache if available
            return self._cache["data"] or {"stocks": [], "cryptos": [], "error": str(e)}
