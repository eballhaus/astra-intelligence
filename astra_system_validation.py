...
LOG_FILE = ROOT / "astra_logs" / f"astra_validation_log_{datetime.now():%Y%m%d_%H%M%S}.txt"
...
if results["Guardian"] == "✅ OK" and results["Engine"] == "✅ OK":
    try:
        import astra_modules.guardian.guardian_v7 as gv7
        import astra_modules.engine.orchestrator as orch
        results["Guardian→Engine link"] = "✅ OK" if hasattr(orch, "Orchestrator") else "⚠️ No Orchestrator class"
    except Exception as e:
