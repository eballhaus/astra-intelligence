...
from core.guardian.scheduler import LearningScheduler
from core.learning.continual_trainer import ContinualTrainer
        ...
        try:
            while self.running:
                if not self.scheduler.is_healthy():
                    self._log("⚠️ Scheduler unhealthy — attempting restart.", "WARNING")
                    self.scheduler.stop()
                    time.sleep(3)
