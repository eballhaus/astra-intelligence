...
from core.api_keys import FMP_API_KEY
from core.guardian.guardian_core import guardian


def get_data(symbol: str) -> pd.DataFrame:
    guardian.log(f"[FMP] 🔄 Fetching data for {symbol}")
    try:
        ...
        df["source"] = "FMP"
        guardian.log(f"[FMP] ✅ Retrieved {len(df)} rows for {symbol}")
        return df

    except Exception as e:
        guardian.log(f"[FMP] ⚠️ Error fetching {symbol}: {e}")
        return pd.DataFrame()
