...
from core.api_keys import EODHD_API_KEY
from core.guardian.guardian_core import guardian


def get_data(symbol: str) -> pd.DataFrame:
    guardian.log(f"[EODHD] 🔄 Fetching data for {symbol}")
    try:
        url = (
        ...
        df["source"] = "EODHD"
        guardian.log(f"[EODHD] ✅ Retrieved {len(df)} rows for {symbol}")
        return df

    except Exception as e:
        guardian.log(f"[EODHD] ⚠️ Error fetching {symbol}: {e}")
        return pd.DataFrame()
