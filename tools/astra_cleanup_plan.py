...
import os, re, sys, json, tarfile, shutil, time, importlib.util, ast, subprocess
...
def find_imports():
    ...
    for pyfile in SAFE_ROOT.rglob("*.py"):
        if "venv" in str(pyfile) or "backup" in str(pyfile): continue
        try:
            ...
            for node in ast.walk(ast.parse(src)):
                if isinstance(node, ast.Import):
                    ...
                    mapping.setdefault(node.module.split(".")[0], []).append(str(pyfile))
...
def auto_fix_imports(old_name, new_pkg):
    ...
    for pyfile in SAFE_ROOT.rglob("*.py"):
        ...
        if pattern.search(txt):
            newtxt = re.sub(
                rf"from\s+([\w\.]*){old_name}",
                rf"from {new_pkg}.{old_name}", txt)
            newtxt = re.sub(
                rf"import\s+{old_name}",
                rf"from {new_pkg} import {old_name}", newtxt)
            if newtxt != txt:
...
def stage_legacy():
    ...
    if legacy.exists(): compress_dir(legacy, "legacy_archive_2025Q4")
    if backups.exists(): compress_dir(backups, "astra_backups_archive_2025Q4")
    return guardian_check()
...
def rollback():
    if not ROLLBACK_PATH.exists():
        print("No rollback data found."); return
...
def main():
    ...
    if input("Proceed with Guardian cleanup? [y/n]: ").lower() == "y":
        if not stage_guardian(mapping):
            print("Guardian cleanup aborted."); return

    if input("Proceed with UI cleanup? [y/n]: ").lower() == "y":
        if not stage_ui(mapping):
            print("UI cleanup aborted."); return
