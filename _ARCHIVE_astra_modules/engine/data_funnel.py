...
def select_top_assets(category="stock", symbols=None, n=6):
    ...
    if not symbols:
        ...
        symbols = (
            ["AAPL", "NVDA", "MSFT", "AMZN", "TSLA", "META", "GOOGL"]
            if category == "stock"
            else ["BTC", "ETH", "SOL", "BNB", "ADA", "AVAX", "XRP"]
        )

    scores = []
    for s in symbols:
        try:
            data = get_data(s, asset_type="crypto" if category !=
                            "stock" else "stock")
...
def get_symbols_for_category(category: str):
    ...
    guardian_log(
        f"[DataFunnel] Mode={mode} | Loaded {len(symbols)} {category} symbols from UniverseBuilder."
    )
    return symbols
