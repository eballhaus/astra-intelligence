...
def get_stock_meta(symbol):
    ...
    try:
        ...
        if dj_key:
            data = rate_safe_get("https://api.datajockey.io/v0/company/financials", params={"ticker": symbol, "apikey": dj_key})
    ...
    try:
        ...
        if fh_key:
            news = rate_safe_get("https://finnhub.io/api/v1/company-news", params={"symbol": symbol, "from":"2024-12-01","to":"2024-12-15","token": fh_key})
...
def get_crypto_meta(symbol):
    ...
    try:
        ...
        if pg_key:
            data = rate_safe_get(f"https://api.polygon.io/v2/aggs/ticker/X:{symbol}USD/prev", params={"apiKey": pg_key})
    ...
    try:
        ...
        if sim_key:
            sdata = rate_safe_get("https://api.simfin.com/api/v2/companies/list", params={"api-key": sim_key})
            result["sentiment"] = len(sdata.get("data", [])) % 100 / 100  # placeholder normalization
...
def get_market_sentiment():
    ...
    result = {}
    try:
        data = rate_safe_get("https://api.alternative.me/fng/", params={"limit": 1})
        fng = data.get("data", [{}])[0]
        result["fear_greed_index"] = int(fng.get("value", 50))
        result["fear_greed_text"] = fng.get("value_classification", "Neutral")
        guardian_log(f"[Hydra:FNG] Index {result['fear_greed_index']} ({result['fear_greed_text']})")
    except Exception as e:
