...
def fetch_index_data() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        try:
            ...
            guardian_log(f"[SummaryCards] ⚠️ AstraAPI init failed: {e}")
            return {}
        ...
        guardian_log(f"[SummaryCards] 🚨 Index fetch critical error: {e}")
        return {}
...
def render_summary() -> None:
    ...
    try:
        ...
        if not index_data:
            guardian_log("[SummaryCards] ⚠️ Using fallback index data")
            index_data = {
                "^GSPC": {
                    "name": "S&P 500",
                    "icon": "💹",
        ...
        guardian_log("[SummaryCards] ✅ Summary cards rendered successfully")

    except Exception as e:
        guardian_log(f"[SummaryCards] 🚨 Render error: {e}")
