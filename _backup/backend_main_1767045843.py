...
def _cache_get(symbol: str) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        if entry and (time.time() - entry["timestamp"]) < _CACHE_TTL:
            guardian.log(f"[BackendCache] ♻️ Cache hit for {symbol}")
            return entry["data"]

        if entry:
            del _CACHE[symbol]
    except Exception as e:
        guardian.log(f"[BackendCache] ⚠️ Cache retrieval error: {e}")
...
def _cache_set(symbol: str, data: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        guardian.log(f"[BackendCache] 💾 Cached {len(data)} records for {symbol}")
    except Exception as e:
        guardian.log(f"[BackendCache] ⚠️ Cache set error: {e}")
...
async def fetch_from_module(api_name: str, symbol: str) -> pd.DataFrame:
    ...
    """
    try:
        module = __import__(f"core.apis.{api_name}", fromlist=["get_data"])
        ...
        if hasattr(module, "get_data_async"):
            ...
            guardian.log(f"[Backend] ⚠️ {api_name} has no valid data method")
            return pd.DataFrame()
        ...
        guardian.log(f"[Backend] ⚠️ {api_name} returned empty DataFrame for {symbol}")

    except Exception as e:
        guardian.log(f"[Backend] ⚠️ {api_name} fetch failed for {symbol}: {e}")
...
async def get_data(symbol: str) -> ORJSONResponse:
    ...
    """
    guardian.log(f"[Backend] 🔄 Fetch request received for {symbol}")
    ...
    if cached is not None:
        guardian.log(f"[Backend] 💾 Cache hit for {symbol} ({len(cached)} records)")
    ...
    try:
        ...
        guardian.log(f"[Backend] 🚀 Attempting AstraAPI v3.0 for {symbol}")
        api = AstraAPI()
        df = api.get_market_data(symbol)

        if df is not None and not df.empty:
            guardian.log(f"[Backend] ✅ AstraAPI provided {len(df)} rows for {symbol}")
            data = df.to_dict(orient="records")
            _cache_set(symbol, data)
            safe_json = orjson.dumps(data, default=_json_safe_default)
            return ORJSONResponse(content=orjson.loads(safe_json))

        guardian.log(f"[Backend] ⚠️ AstraAPI empty for {symbol}, falling back")

    except Exception as e:
        guardian.log(f"[Backend] ⚠️ AstraAPI failed: {e}")
    ...
    try:
        ...
        guardian.log(f"[Backend] 🚨 Async gather error: {e}")
        results = []
    ...
    if not dfs or all(df.empty for df in dfs):
        ...
        guardian.log(f"[Backend] 🧩 Mock data served for {symbol} ({len(data)} rows)")
        safe_json = orjson.dumps(data, default=_json_safe_default)
        return ORJSONResponse(content=orjson.loads(safe_json))
    ...
    try:
        ...
        guardian.log(f"[Backend] ⚠️ Guardian quota tracking skipped: {e}")
    ...
    try:
        ...
        _cache_set(symbol, data)
        guardian.log(f"[Backend] ✅ Served {len(df)} normalized records for {symbol}")

        safe_json = orjson.dumps(data, default=_json_safe_default)
        return ORJSONResponse(content=orjson.loads(safe_json))

    except Exception as e:
        guardian.log(f"[Backend] 🚨 JSON serialization error for {symbol}: {e}")
        return ORJSONResponse(
