...
from core.api_keys import ALPHA_VANTAGE_API_KEY
from core.guardian.guardian_v6 import guardian


def get_data(symbol: str) -> pd.DataFrame:
    guardian.log(f"[AlphaVantage] 🔄 Fetching data for {symbol}")
    try:
        ...
        guardian.log(f"[AlphaVantage] ✅ Retrieved {len(df)} rows for {symbol}")
        return df

    except Exception as e:
        guardian.log(f"[AlphaVantage] ⚠️ Error fetching {symbol}: {e}")
        return pd.DataFrame()
