...
try:
    ...
    def guardian_boot(*args, **kwargs):
        print(
            "[GuardianCompat] 🧠 Legacy guardian_boot() called — initializing guardian_log."
        )
        return guardian_log()
...
try:
    if not hasattr(guardian_log, "log") or guardian_log.log.__code__.co_argcount < 2:
        ...
        builtins.guardian_log = guardian_log
        print(
            "[GuardianCompat] ✅ guardian_log.log(message) method patched for compatibility."
        )
except Exception as e:
...
try:
    ...
    if cls:
        ...
        print(
            "[GuardianCompat] ✅ guardian_log.log(self, message) patched successfully."
        )
    else:
...
try:
    ...
    if isinstance(builtins.guardian_log, object) and not isinstance(
        builtins.guardian_log, type
    ):
        ...
        if real_guardian_class:
            ...
            print(
                "[GuardianCompat] ✅ Restored guardian_log as callable class (was instance)."
            )
        else:
            print(
                "[GuardianCompat] ⚠️ guardian_log restoration skipped (already callable)."
            )
except Exception as e:
...
try:
    ...
    if not isinstance(builtins.guardian_log, type):
        ...
        if "guardian_log" in globals() and isinstance(globals()["guardian_log"], type):
            ...
            print(
                "[GuardianCompat] ✅ Repaired: guardian_log restored as true class definition."
            )
        else:
            print(
                "[GuardianCompat] ⚠️ Unable to locate real guardian_log class in globals()."
            )
    else:
        print(
            "[GuardianCompat] 🧠 guardian_log is already callable (no repair needed)."
        )
except Exception as e:
...
def _restore_guardian_class():
    ...
    if not inspect.isclass(builtins.guardian_log):
        ...
        print(
            "[GuardianCompat] ✅ Hard reset — guardian_log restored as callable class."
        )
    else:
...
def _deferred_guardian_fix():
    ...
    try:
        ...
        if not inspect.isclass(real_class):
            print(
                "[GuardianCompat] ⚠️ Deferred fix — guardian_log class not yet loaded."
            )
            return
        ...
        if inspect.isclass(getattr(builtins, "guardian_log", None)):
            print(
                "[GuardianCompat] 🧠 guardian_log is already callable (class intact)."
            )
            return
        ...
        builtins.guardian_log = real_class
        print(
            "[GuardianCompat] ✅ Deferred fix — guardian_log restored as callable class."
        )
    except Exception as e:
...
def _watch_guardian_class():
        ...
        try:
            ...
            if inspect.isclass(real_class):
                if not inspect.isclass(getattr(builtins, "guardian_log", None)):
                    builtins.guardian_log = real_class
                    print(
                        "[GuardianCompat] ✅ Watchdog — guardian_log restored as callable class."
                    )
                else:
                    print(
                        "[GuardianCompat] 🧠 Watchdog — guardian_log already callable."
                    )
                restored = True
                break
        except Exception as e:
    ...
    if not restored:
        print(
            "[GuardianCompat] ❌ Watchdog timeout — guardian_log class not found after wait."
        )
...
if not inspect.isclass(getattr(builtins, "guardian_log", None)):
    ...
    class guardian_log:
        ...
        def __init__(self, *args, **kwargs):
            ...
            msg = (
                args[0]
                if args
                else "[GuardianBootstrap] guardian_log initialized early."
            )
            print(msg)
            self.messages.append(msg)
    ...
    builtins.guardian_log = guardian_log
    print(
        "[GuardianCompat] ✅ Bootstrap — guardian_log class reinstated before imports."
    )
else:
    print(
        "[GuardianCompat] 🧠 guardian_log class already intact (no bootstrap needed)."
    )

# --- Phase7 Compatibility Patch ---
try:
    GuardianV6 = guardian_log
except NameError:
    from astra_core.guardian.guardian_v6 import guardian_log as GuardianV6
