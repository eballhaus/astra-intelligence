...
if os.path.exists(legacy_path) and "astra_modules" not in sys.modules:
    ...
    spec = (
        importlib.util.spec_from_file_location(
            "astra_modules", os.path.join(legacy_path, "__init__.py")
        )
        if os.path.exists(os.path.join(legacy_path, "__init__.py"))
        else None
    )
    ...
    if spec and astra_modules:
        ...
        print(
            "[AstraCompat] ✅ 'astra_modules' alias registered for astra_modules_backup_20251130_1720"
        )
